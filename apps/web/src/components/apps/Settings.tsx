...
export function SettingsApp() {
	...
	const categories = [
		{
			name: "Personalização",
			icon: Palette,
			description: "Tela de Fundo, tela de bloqueio, cores",
		},
		{
			name: "Contas",
			icon: User,
			description: "Suas contas, email, sincronização, trabalho, família",
		},
	...
	return (
		<div
			className={`h-full w-full bg-black text-white overflow-auto transition-all duration-500 ease-in-out ${
		...
		>
			...
			<div>
				...
				<div className="bg-[#171717] flex flex-row justify-between items-center gap-4 py-8">
					<div
						className={`w-full flex items-center justify-center gap-4 col-span-1 mb-8 transition-all duration-700 delay-100 ${
							isVisible
					...
					<div
						className={`w-full flex items-baseline justify-center gap-8 col-span-1 mb-8 transition-all duration-700 delay-300 ${
							isVisible
				...
				<div className="bg-black flex flex-col items-center gap-4 justify-center pt-8">
					<div
						className={`relative mb-8 min-w-lg transition-all duration-700 delay-200 ${
							isVisible
								? "opacity-100 translate-y-0"
								: "opacity-0 translate-y-4"
						}`}
				...
				<div
					className={`bg-black grid grid-cols-5 max-w-7xl mx-auto place-items-center gap-6 p-4 py-8 transition-all duration-700 delay-400 ${
