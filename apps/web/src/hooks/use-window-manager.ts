...
export const useWindowManager = create<WindowManagerState>((set, get) => ({
   ...
   updateWindowPosition: (id, position) => {
      ...
      const constrainedPosition = constrainToBoundaries(position, window.size.width, window.size.height);

      set((state) => ({
         windows: state.windows.map((w) => (w.id === id ? { ...w, position: constrainedPosition } : w)),
      }));
   },
   ...
   updateWindowSize: (id, size) => {
      ...
      const constrainedPosition = constrainToBoundaries(window.position, size.width, size.height);

      set((state) => ({
         windows: state.windows.map((w) => (w.id === id ? { ...w, size, position: constrainedPosition } : w)),
      }));
   },
